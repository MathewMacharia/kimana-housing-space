...
const PaymentModal: React.FC<PaymentModalProps> = ({ onClose, onSuccess, title, amount, subtitle, listingId, userEmail }) => {
  ...
  // Listen for transaction status changes and poll Daraja API automatically (Step 2)
  useEffect(() => {
    if (!checkoutId || step !== 'processing') return;

    // 1. Listen to Firestore for Webhook updates
    ...
    // 2. Fallback: Automatically poll Daraja STK Query API every 5 seconds
    // because Daraja Sandbox webhooks are frequently delayed or dropped.
    const pollInterval = setInterval(async () => {
      try {
        ...
        // If it throws an error, it just means it's still pending or failed.
        // We do not set error state immediately to allow the user time to enter the PIN.
        // Only log to console.
        console.log("Polling status:", e.message);
      }
    }, 5000);
  ...
  }, [checkoutId, step, onSuccess, listingId]);
