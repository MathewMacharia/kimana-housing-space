...
const Settings: React.FC<SettingsProps> = ({
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.email !== currentUser.email) {
        message += "\n\nA verification link has been sent to your new email. Please verify it to complete the change.";
      }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom duration-500 pb-10">
      ...
      <SettingRow icon="fa-language" label={t('language')} action={
      ...
      <SettingRow icon="fa-moon" label={t('theme')} action={
